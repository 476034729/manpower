    ...
    <nav class="navbar-default navbar-static-side" role="navigation" style="z-index: 1991;">
        ...
        <div class="sidebar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <%--<t:hrmMenu style="hplus" menuFun="${menuMap}"></t:hrmMenu>--%>
...
<script>
    ...
    $(document).ready(function () {
        $.ajax({
            async : false,
            type:"get",
            url:"hrmMenuController.do?getMenuByUser",
            cache:false,
            success:function (result) {
                var r = $.parseJSON(result);
             $.each(r,function (index,menu) {
                 var li=$("<li></li>");
                 var outside=$("<a></a>").attr("href","#");
                 var i=$("<i></i>").addClass("fa fa-columns");
                 var span=$("<span></span>").addClass("menu-text").append(index);
                 var fa=$("<span></span>").addClass("fa arrow");
                 i.appendTo(outside);
                 span.appendTo(outside);
                 fa.appendTo(outside);
                 li.append(outside);
                 build_child_menu(menu,li);
    ...
    function build_child_menu(menus , parent) {

        var ul=$("<ul></ul>").addClass("nav nav-second-level collapse").attr("aria-expanded","false").attr("style","height: 0px;");
        $.each(menus,function (index,menu) {
            var li= $("<li></li>").append($("<a></a>").addClass("J_menuItem").attr("href","#"+menu.menu_url).attr("data-index",index).append(menu.menu_name));
            li.appendTo(ul);
        })
