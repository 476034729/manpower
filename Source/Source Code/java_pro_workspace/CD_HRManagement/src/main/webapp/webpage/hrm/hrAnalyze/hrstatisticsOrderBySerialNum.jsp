...
<script>
    ...
    function build_table_twice(year, month) {
        $.ajax({
            type: 'get',
            url: 'HrCMAnalyzeController.do?getstatisticsinfoOrderBySerialNum',
            data: {
                'year': year,
                'month': month
            },
            ...
            success: function (result) {
                ...
                $.each(list, function (index1, item1) {
                    ...
                    var totalTr = $("<tr></tr>").attr("id", "total" + index1).append($("<td></td>").append(serinalNum)).append($("<td></td>").append(index1))
                        .append($("<td></td>").append("总共"));

                    // var amTr = $("<tr></tr>").attr("id", "total" + index1).append($("<td></td>").attr("rowspan", "2").attr("colspan", "2").append(index1))
                    //     .append($("<td></td>").append("上午"));
                    // var pmTr = $("<tr></tr>").attr("id", "pm" + index1).append($("<td></td>").append("下午"));
                    totalTr.appendTo($("#tbody"));
                    // pmTr.appendTo($("#tbody"));
                    $("<td></td>").attr("id", "totalRate" + name).appendTo($("#total" + name));
                    // $("<td></td>").attr("id", "pmRate" + name).appendTo($("#pm" + name));
                    $.each(codeList, function (index2, item2) {

                        $("<td></td>").attr("id", "total" + item2.code + name).appendTo($("#total" + name));
