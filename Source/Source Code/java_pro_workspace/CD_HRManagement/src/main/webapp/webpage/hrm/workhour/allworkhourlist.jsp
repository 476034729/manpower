...
<div id="head_select" style="">
    ...
    <div style="display:inline-block;margin-left: 10px" id="userContent">
        <span style="color: white">员工名：</span> <select id="Ste_user_selector" style=""></select>
    </div>
...
<div  class="btn_option" id="button1">
...
<script>
    ...
    $("#query").click(function () {
        ...
        $("#work_hour_table").show();
   		$("#button1").show();
   		first(ids,queryYear,queryMonth);
    });
    ...
    function first(ids,queryYear,queryMonth){
    	 $.ajax({
             type: 'get',
             ...
             success: function (result) {
                 ...
                 build_table_once(ids,queryYear,queryMonth);
             }
         })
    }
   ...
   // <!-- 建立table基本信息 -->
    function build_table_once(ids,queryYear,queryMonth) {
        $.ajax({
            type: 'get',
            url: 'WorkHourController.do?showlist&year=' + queryYear + '&month= ' + queryMonth,
            async: false,
            cache: false,
            success: function (result) {
                r = JSON.parse(result);
   ...
   // <!-- 将工时信息对table再一次补全 -->
    function build_table_twice(ids,queryYear,queryMonth) {
        $.ajax({
            type: 'get',
            url: 'WorkHourController.do?getcmworkhourlist&year=' + queryYear + '&month=' + queryMonth+'&id='+ids,
            async: false,
            cache: false,
            success: function (result) {
                r = JSON.parse(result);
   ...
   // <!-- 提交按钮事件 -->
    $("#submit").click(function () {
        ...
        $.ajax({
            type: 'post',
            url: 'WorkHourController.do?submitcmworkhourlist',
            data: {
   ...
   /*  $("#export").click(function () {
    	var select = document.getElementById("Ste_user_selector");
        ...
        $('#work_hour_table').tableExport({
            type: 'excel',
            escape: 'false',
            fileName: queryYear + '/' + queryMonth +selectedText+ '工时信息'
        });
    }); */
</script>
</body>
