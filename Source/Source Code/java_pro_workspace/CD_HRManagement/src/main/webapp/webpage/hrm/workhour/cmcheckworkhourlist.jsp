    ...
    <div style="display:inline-block;margin-left: 10px" id="userContent">
     ...
     <div style="display:inline-block;margin-left: 10px" >
        <span style="color: white">状态：</span>
	    	<select id="Ste_status_selector" style="" multiple="multiple">
        		    <option value="0">未填写完</option>
                    <option value="1">已填写完</option>
       	    </select>
    </div>
     <div style="display:inline-block;margin-left: 10px">
        <span style="color: white">项目名：</span> <select id="Ste_project_selector" style="" multiple="multiple"></select>
    </div>
...
<script>
   ...
   // <!-- -->
        ...
        $.ajax({
        	...
        	success:function(result){
                ...
                $.each(list, function (index, item) {
                	//alert(item.id);
                    $("<option></option>").attr("value", item.code).append(item.project_name).appendTo($("#Ste_project_selector"));
                })
            }
        })
        $("#Ste_status_selector").multiselect("destroy").multiselect({
            buttonWidth: 160,  //选择框的大小
            maxHeight: 140,
            includeSelectAllOption: true//是否现实全选
        });
          $("#Ste_project_selector").multiselect("destroy").multiselect({
            buttonWidth: 160,  //选择框的大小
            maxHeight: 200,
            includeSelectAllOption: true//是否现实全选
        });
    ...
    //<!-- 将工时信息对table再一次补全 -->
    function build_table_twice(statuss,projects,ids, year, month) {
        $.ajax({
            type: 'get',
            url: 'WorkHourController.do?cmCheck',
            data: {
            ...
            success: function (result) {
                ...
                $.each(r, function (index, item) {
                    ...
                    $.each(WorkHourList, function (index2, item2) {
                        ...
                        console.log(item2)
                        if (!$.trim(item2.project_code)) {console.log(111)
                            ...
                            if("其他"==projectName){
                            	 $(id + 'CodeTd' + name).css("color","blue").append("<div class='td_item'>"+projectName+"</div>")
                            }else{
    ...
    $("#query").click(function () {
        ...
        var statuss =$("#Ste_status_selector").val();
        var projects = $("#Ste_project_selector").val();
        
        queryYear = time.split("-")[0];
        queryMonth = time.split("-")[1];
        build_table_once(queryYear, queryMonth);
        build_table_twice(statuss,projects,ids, queryYear, queryMonth);
    })
    ...
    $("#timeSelector").change(function () {
        ...
        $.ajax({
            type: 'get',
            url: 'WorkHourController.do?getstealluserlist',
            data: {
                'year': queryYear,
                'month': queryMonth
            },
