...
<div id="toolbar" style="padding: 5px">
    ...
    <label for="userName" style="display:${roleType==0?"none":''}">申请人</label>
    <input type="text" id="userName" name="userName" style="display: ${roleType==0?"none":''}">
    <label >申请日期</label>
    <input type="date" id="startDate" name="startDate">-
    <input type="date" id="endDate" name="endDate">
    <a onclick="query()" href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true">查询</a>
    <button class="btn-primary"><a  href="attendance.do?vacationApply" style="text-decoration: none;color: white">添加申请</a></button>
</div>
<script>
    $(document).ready(function () {
        //配置datagrid
        $("#table").datagrid({loadFilter:pagerFilter}).datagrid({
            url:"attendance.do?queryvacationApply",
            method:"get",
            pagination:true,
            pageSize:10,
            ...
            columns:[[{
                ...
                formatter:function(value,rowData,rowIndex){
                    if(rowData.division_manager_examine==0){
                        ...
                        return "不同意";
                    }else if(rowData.division_manager_examine==3){
                        return "不审核";
                    }
                }
            },{
    ...
    function formatOper(val,row,index) {
        var isResponsPerson = $("#roleType").val();
        var ss = '<a class="operate" href="attendance.do?vacationApplyDetail&vacationApplyId='+row.id+'" >详情</a>';
        var userId = $("#userId").val();
        if(row.status==2 && userId==row.user_id){
            ss = ss + ' <a class="operate" href="attendance.do?vacationApply&changeofrestId='+row.id+'">修改</a>';
        }
        if(userId==row.user_id){
            if(row.user_status==0){
                if(row.immediate_supervisor_examine==1&&(row.division_manager_examine==1||row.division_manager_examine==3)){
                    ss = ss + ' <a class="operate" href="#" onclick="confirmVacation(\''+row.id+'\')">确认</a>';
                }
        ...
        if(isResponsPerson==1){
            if(row.user_status==0){
                if((row.immediate_supervisor_examine==0||row.immediate_supervisor_examine==2)&&row.division_manager_examine!=1&&row.division_manager_examine!=2){
                    if(row.status==2){
                        ss = ss + ' <c:if test="${roleType!=0&&roleType!=3}"><a class="operate" href="attendance.do?vacationApplyExamination&vacationApplyId='+row.id+'">重审</a></c:if> ';
                    }else{
                        ss = ss + ' <c:if test="${roleType!=0&&roleType!=3}"><a class="operate" href="attendance.do?vacationApplyExamination&vacationApplyId='+row.id+'">审批</a></c:if> ';
                    }
                }
            }
        }
        ...
        if(isResponsPerson==2){
            if(row.user_status==0&&(row.division_manager_examine==0||row.division_manager_examine==2)){
                if(row.status==2){
                    ss = ss + ' <c:if test="${roleType!=0&&roleType!=3}"><a class="operate" href="attendance.do?vacationApplyExamination&vacationApplyId='+row.id+'">重审</a></c:if> ';
                }else{
                    ss = ss + ' <c:if test="${roleType!=0&&roleType!=3}"><a class="operate" href="attendance.do?vacationApplyExamination&vacationApplyId='+row.id+'">审批</a></c:if> ';
    ...
    //用户确认加班
    function confirmVacation(id) {
        var r = confirm("是否确认换休？确认后将提交给人事。");
        if(r == true) {
            $.ajax({
                type: 'get',
                url: 'attendance.do?userConfirmVacationApply&vacationApplyId=' + id,
                async: false,
                cache: false,
                success: function (result) {
                    var resultDate = JSON.parse(result);
    ...
    function adminConfirm(id) {
        ...
        if(r == true){
            $.ajax({
                type:'get',
                url:'attendance.do?adminConfirmVacationApply&vacationApplyId='+id,
                async:false,
                cache:false,
                success:function(result){
                    var resultDate = JSON.parse(result);
