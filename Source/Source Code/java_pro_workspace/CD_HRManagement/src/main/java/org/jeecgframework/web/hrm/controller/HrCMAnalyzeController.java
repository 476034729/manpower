...
@Controller
@RequestMapping("/HrCMAnalyzeController")
public class HrCMAnalyzeController {

    private static final org.apache.log4j.Logger logger= org.apache.log4j.Logger.getLogger(HrCMAnalyzeController.class);

    @Autowired
    private HrCMStatisticsService hrCMStatisticsService;

    @RequestMapping(params = "gohrstatisticslist")
    public ModelAndView goHrStatisticsList(){
        return new ModelAndView("hrm/hrAnalyze/hrstatisticsCM");
    }

    @RequestMapping(params = "gohrstatisticslistOrderBySerialNum")
    public ModelAndView goHrStatisticsListOrderBySerialNum(){
        return new ModelAndView("hrm/hrAnalyze/hrstatisticsOrderBySerialNum");
    }
    ...
    public List<ProjectCode> getProjectList(){
        return hrCMStatisticsService.getProjectList();
    }

    /**
     * @Author Eale
     * @Description 返回排序后的统计
     * @Date 10:55 2019/4/27
     * @Param [year, month]
     * @return org.jeecgframework.core.common.model.json.AjaxJson
     **/
    @RequestMapping(params = "getstatisticsinfoOrderBySerialNum")
    @ResponseBody
    public AjaxJson getStatisticsInfoOrderBySerialNum(int year , int month){
        logger.info(">>getStatisticsInfoOrderBySerialNum：Start query year:"+year+",month:"+month);
        ...
        if (!hrCMStatisticsService.hasRecord(year,month)){
            a.setSuccess(false);
            a.setMsg("没有该月数据!");
            return a;
        }

        List<TSUser> users = hrCMStatisticsService.getUserByWorkHour(year,month);
...
//
//        }
        a.setObj(hrCMStatisticsService.getTimeOfCodeOrderBySerialNum(year,month,users));
...
//        hrCMStatisticsService.getTimeOfCode(year,month);


        //mark
//        for (TSUser user :users){
//            Map<String,Object> newMap = new HashMap<String, Object>();
//            newMap.putAll(hrCMStatisticsService.getTimeOfCode(year,month,user.getId()));
//            newMap.putAll(hrCMStatisticsService.getRateOfAttendance(year,month,user.getId()));
//            map.put(user.getUserName(),newMap);
//
//        }


        a.setObj(hrCMStatisticsService.getTimeOfCode(year,month,users));
    ...
    public Map<String,Map<String,Object>> getTotalInfo(int year,int month){
        ...
        map.put("ProjectPeopleDay",hrCMStatisticsService.getCodePeopleTimes(year,month));
        map.put("ProjectPeopleDayRate",hrCMStatisticsService.getProjectPeopleDayRate(year,month));
        return map;
    }

    @RequestMapping(params = "gohranalyzemap")
    public ModelAndView goHrAnalyzeMap(){
        return new ModelAndView("hrm/hrAnalyze/analyzemapCM");
    }
    ...
    public AjaxJson getProjectPeopleDayRate(int year ,int month){
        ...
        try{
        	...
        	Map<String, Object> map =hrCMStatisticsService.getProjectPeopleDayRate(year,month);
        	if(map==null) {
        		a.setSuccess(false);
                a.setMsg("没有该月数据");
                return a;
    ...
    public void exportMapExcel(HttpServletRequest request,HttpServletResponse response,int year , int month ) throws Exception{
        Map<String,Object> map = hrCMStatisticsService.getProjectPeopleDayRate(year,month);
        List<ProjectCode> list =hrCMStatisticsService.getProjectList();
        ...
        HSSFWorkbook hssfWorkbook = hrCMStatisticsService.getHSSFWorkbook(year+"年"+month +"月人力分析图表",projectList,rateList);
