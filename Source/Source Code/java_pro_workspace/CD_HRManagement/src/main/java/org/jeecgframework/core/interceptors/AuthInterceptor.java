...
public class AuthInterceptor implements HandlerInterceptor {
	...
	private static List<HrmMenu> hrmMenus;
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && (client.getUser()!=null ||client.getHrmUser()!=null )) {
				...
				List<HrmMenu> hrmMenus=hrmMenuService.findHql("from HrmMenu u where 1=1");
				if (functions.size()>0){
					functionId = functions.get(0).getId();
				}

				if (hrmMenus.size()>0){
					menuId=hrmMenus.get(0).getMenu_id();
				}
...
//			functionList = systemService.loadAll(TSFunction.class);
		...
		if (hrmMenus == null){
			hrmMenus = hrmMenuService.findHql("from HrmMenu u where 1=1");
		}
