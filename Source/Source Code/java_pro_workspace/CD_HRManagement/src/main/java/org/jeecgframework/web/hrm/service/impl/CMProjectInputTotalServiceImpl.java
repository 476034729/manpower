...
@Service("CMProjectInputTotalService")
@Transactional
public class CMProjectInputTotalServiceImpl extends CommonServiceImpl implements CMProjectInputTotalService {

    private static final org.apache.log4j.Logger logger= org.apache.log4j.Logger.getLogger(CMProjectInputTotalServiceImpl.class);


    @Override
    public List<TSUser> getUserList() {
        String hql = "from TSUser u where u.userstatus = '在职' and u.jobNum != ' ' and u.site = 'CD' and u.level != 'M100000' order by u.level desc";
        logger.info(hql);
    ...
    public List<Map<String, Object>> getWorkHourByProject(String code, int year, int month) {
        ...
        logger.info(">>CardingRecordQueryServiceImpl:getWorkHourByProject: "+new Date().toString());

        String userHql = "from WorkHourCM u where 1=1 and YEAR(u.work_day)=? and MONTH(u.work_day) =? and u.project_code = ?";
        List<Map<String,Object>> mapList=new ArrayList<Map<String, Object>>();
        List<WorkHourCM> list=this.findHql(userHql,year,month,code);
    ...
    public Map<String, Object> getUserWorkHour(String code, String id,int year ,int month) {
        ...
        String hql = "from WorkHourCM u where 1=1 and YEAR(u.work_day)=? and MONTH(u.work_day) =? and u.project_code =? and u.user_id =?";
        Map<String,Object> map = new HashMap<String, Object>();
        List<WorkHourCM> list = this.findHql(hql,year,month,code,id);
        map.put("id",id);
    ...
    public Map<String, Object> getWorkHourCount(String code, int year, int month) {
        ...
        logger.info(">>CardingRecordQueryServiceImpl:getWorkHourCount: "+new Date().toString());
        ...
        String userHql = "from WorkHourCM u where 1=1 and YEAR(u.work_day)=? and MONTH(u.work_day) =?";

        String hql = "from TSUser u where u.id = ? and u.jobNum != ' ' and u.site = 'CD'";

        List<WorkHourCM> list=this.findHql(userHql,year,month);
        Set<String> stringSet = new HashSet<String>();
        ...
        for (String s :strings) {
            String sql = "select count(1) from t_hrm_workhour_cm a " +
                    "join t_hrm_project_code b  on a.project_code = b.code " +
                    "join t_s_user c on c.id = a.user_id  where YEAR(a.work_day)=" + year + " and MONTH(a.work_day) = " + month + " and c.department = '" + s + "' and b.code =  '"+ code +"'";
