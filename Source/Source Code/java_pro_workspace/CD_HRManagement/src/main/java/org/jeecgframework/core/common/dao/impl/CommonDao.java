...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
	...
	@Override
	public HrmUser checkUser(HrmUser hrmUser) {
		String password = PasswordUtil.encrypt(hrmUser.getJob_num(), hrmUser.getPassword(), PasswordUtil.getStaticSalt());
		String query = "from HrmUser u where u.job_num = :job_num and u.password=:password";
		Query queryObject = getSession().createQuery(query);
		queryObject.setParameter("job_num", hrmUser.getJob_num());
		queryObject.setParameter("password", password);
		List<HrmUser> hrmUsers = queryObject.list();

		if (hrmUsers != null && hrmUsers.size() > 0) {
			return hrmUsers.get(0);
		} else {
			queryObject = getSession().createQuery(query);
			queryObject.setParameter("job_num", hrmUser.getJob_num());
			queryObject.setParameter("password", hrmUser.getPassword());
			hrmUsers = queryObject.list();
			if(hrmUsers != null && hrmUsers.size() > 0){
				return hrmUsers.get(0);
			}
		}
